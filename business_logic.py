...
def generate_mock_data():
    ...
    for i in range(MOCK_DATA_SIZE["customers"]):
        customer = {
            ...
            "joined_date": (datetime.now() - timedelta(days=random.randint(0, 7))).isoformat()
        }
    ...
    for i in range(MOCK_DATA_SIZE["appointments"]):
        ...
        appointment = {
            ...
            "service": random.choice(["Consultation", "Follow-up", "Review", "Planning"]),
    ...
    for customer in sample_customers:
        ...
        customer_appointments = [a for a in appointments if a['customer_id'] == customer['id']]
...
async def get_customer(phone=None, email=None, customer_id=None):
    ...
    if phone:
        customer = next((c for c in MOCK_DATA["customers"] if c["phone"] == phone), None)
    elif email:
        customer = next((c for c in MOCK_DATA["customers"] if c["email"] == email), None)
    elif customer_id:
...
async def get_customer_appointments(customer_id):
    ...
    appointments = [a for a in MOCK_DATA["appointments"] if a["customer_id"] == customer_id]
    return {
